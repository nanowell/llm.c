...
#define GELU_COEF (GELU_SCALING_FACTOR * 0.044715f)
#define GELU_CUBE_COEF (3.0f * 0.044715f)

__global__ void gelu_forward_kernel(floatX* out, const floatX* inp) {
...
__global__ void gelu_backward_inplace_kernel(floatX* d_in_out, const floatX* inp) {
    ...
    for (int k = 0; k < packed_inp.size; ++k) {
        ...
        float cube = GELU_COEF * x * x * x;
        float tanh_arg = GELU_SCALING_FACTOR * (x + cube);
        float tanh_out = tanhf(tanh_arg);
        float coshf_out = coshf(tanh_arg);
        float sech_out = 1.0f / (coshf_out * coshf_out);
        float local_grad = 0.5f * (1.0f + tanh_out) + x * 0.5f * sech_out * GELU_SCALING_FACTOR * (1.0f + GELU_CUBE_COEF * x * x);
...
void gelu_forward(floatX* out, const floatX* inp, int N, cudaStream_t stream) {
    ...
    gelu_forward_kernel<<<grid_size, block_size, 0, stream>>>(out, inp);
